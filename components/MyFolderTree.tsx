...
export default function MyFolderTree() {
        ...
        return (
            <ul>
                {nodes.map((node, index) => (
                    <li className={`${node.type === 'directory' ? "cursor-not-allowed pl-3" : "cursor-pointer hover:bg-gray-600 transition-colors duration-200 rounded-sm pl-3 py-0.5"} ${currentFile.name === node.name ? "bg-gray-700" : ""}`} key={index} onClick={() => handleClick(node)}>
                        <span className="flex text-white" style={{ marginLeft: depth * 20 }}>
                            {node.children ?
                                ...
                                node.name.endsWith('.js') ?
                                    ...
                                    node.name.endsWith('.txt') ?
                                        ...
                                        node.name.endsWith(".json") ? <img className="size-6" src="https://img.icons8.com/glyph-neue/64/FFFFFF/json.png" alt="json" /> :
                                            node.name.endsWith(".ts") ? <svg className="size-6" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 48 48">
    ...
    return <div className="bg-[#1e1f22] min-h-96 w-72 rounded-lg" style={{ height: `${window.innerHeight-45}px`}}>
        <div className="flex justify-between px-6 py-4 gap-10">
            ...
            </button> : <button onClick={() => setFloat(!float)} className="px-3 text-white flex gap-2 bg-[#1d1e22] rounded-md outline-1 outline-double py-1 text-sm hover:bg-gray-800 transition-colors duration-300">
                Add File
                <img className="size-5" src="https://img.icons8.com/pastel-glyph/64/FFFFFF/add-file--v2.png" alt="add-file--v2" />
            </button>}
        </div>
        ...
        <form className="max-w-md mx-auto rounded-md flex justify-center text-white">
            <div className="relative">
                ...
                <input onChange={(e) => setFilter(e.target.value)} type="search" className="block h-10 p-4 ps-10 text-sm focus:border rounded-lg bg-[#151514] text-white" placeholder="Search Files" />
            </div>
        </form>

        <div>
            <h2 className="text-white flex items-center pl-7 pt-2">
                <img className="w-3 h-5" src="https://img.icons8.com/ios/50/FFFFFF/sort-down.png" alt="sort-down" />
                <span className="ml-1">User</span>
            </h2>
        </div>
        <div className="px-7">
            {float ? <div className="relative">
                <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                    <img className="size-5" src="https://img.icons8.com/ios/50/FFFFFF/new-file.png" alt="new-file" />
                </div>
